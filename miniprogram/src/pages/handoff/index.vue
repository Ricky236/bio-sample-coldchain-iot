...
async function issueQr() {
  ...
  qr.value = await taskService.createHandoffQr(taskId.value)
...
async function bootstrap(query?: Record<string, string | undefined>) {
  ...
  try {
    ...
    if (token) await acceptToken(token)
      ...
      await loadTask()
      if (String(task.value?.owner_user_id || '') === String(sessionStore.user?.id || '')) await issueQr()
...
<template>
  <view class="page handoff-page">
    ...
    <template v-else-if="task">
      <view class="handoff-head"><view>▤　运单号：{{ task.task_id }}</view><text>{{ isIssuer ? '发出交接' : '接收交接' }}</text></view>
      ...
      <view class="people">
        <view><label>交出人</label><view class="face" :class="{ verified: issuerVerified }" @tap="isIssuer && openCamera()">{{ isIssuer ? sessionStore.user?.name?.slice(0,1) : '发' }}</view><b>{{ isIssuer ? sessionStore.user?.name : task.sender }}</b><text :class="{ok:issuerVerified}">{{ issuerVerified ? '检测通过' : isIssuer ? '点击检测人脸' : '等待交出人检测' }}</text><p>登录身份 + 摄像头正脸检测</p></view>
        <view><label>接收人</label><view class="face" :class="{ verified: recipientVerified }" @tap="!isIssuer && openCamera()">{{ !isIssuer ? sessionStore.user?.name?.slice(0,1) : task.carrier.slice(0,1) }}</view><b>{{ !isIssuer ? sessionStore.user?.name : task.carrier }}</b><text :class="{ok:recipientVerified}">{{ recipientVerified ? '检测通过' : isIssuer ? '等待扫码检测' : '点击检测人脸' }}</text><p>接收账号需先扫描动态码</p></view>
      </view>
      ...
      <view class="card check-card">
        ...
        <view class="check-row"><view class="check-icon" :class="{warning:!recipientVerified}">{{ recipientVerified?'✓':'!' }}</view><view class="check-content"><view class="check-label">承运人员</view><view class="check-value">{{ task.carrier }} · {{ recipientVerified?'身份已核验':'等待扫码' }}</view></view></view>
      ...
      <button v-if="isIssuer" class="primary submit-button" :disabled="currentFaceVerified" @tap="openCamera">{{ currentFaceVerified ? '交出人人脸已通过，等待接收方确认' : '开始交出人人脸检测' }}</button>
      <button v-else class="primary submit-button" :disabled="submitting" @tap="currentFaceVerified ? verifyOrConfirm() : openCamera()">{{ ready ? '确认接收并转移责任' : currentFaceVerified ? '等待交出人人脸核验' : '开始接收人人脸检测' }}</button>
